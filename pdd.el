            ...
            (lambda (err)
              (unless (memq abort-flag '(cancel abort))
                ...
                (if (and (cl-plusp max-retry) (funcall pdd-retry-condition err))
                  ...
                  (unwind-protect
                      (if (pdd-task-p task)
                          ;; [async] Handle error for the task (Promise)
                          (if (not (eq fail-user 'unset))
                              (condition-case err1
                                  (let ((value (pdd-funcall fail-user (list :error err :request request :text (cadr err) :code (car-safe (cddr err))))))
                                ...
                                (error
                                 ...
                                 (pdd-reject task err1 (pdd--capture-dynamic-context))))
                            (progn
