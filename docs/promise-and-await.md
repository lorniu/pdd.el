...
*  `pdd-let*`: Provides syntax sugar for a more synchronous-looking style when working with tasks, just like what ts/c# done.
...
```emacs-lisp
(pdd-let* ((url1 (await (pdd-delay-task 3 "https://httpbin.org/ip")))
           (res1 (await (pdd url1)))
...
```emacs-lisp
(pdd-let* ((t1 (my-url-retrieve "https://httpbin.org/ip"))
...
## Example 5. schedule tasks + await (pdd-interval-task)
...
## Example 6. the more complex the logic, the more concise the code appears
